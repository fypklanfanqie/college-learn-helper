...
import com.zhiwei.math.glass.ProvideAppGlass
import com.zhiwei.math.ui.theme.ZhiweiTheme
import org.koin.compose.koinInject

/**
 * 应用根：消费外观设置（日夜/字体/震动）与玻璃设置（模式/10 参数/崩溃禁用标记），
 * 向下提供玻璃上下文（ProvideAppGlass 只发 CompositionLocals；
 * 每屏 GlassHost 负责录制，玻璃与内容互为兄弟节点）。
 */
@Composable
fun AppRoot(settings: SettingsStore = koinInject()) {
    ...
    val liquidDisabled by settings.liquidGlassDisabled.collectAsState(initial = false)
